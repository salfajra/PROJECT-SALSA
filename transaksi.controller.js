...
exports.showFormTransaksi = async (request, response) => {
    try {
        /**ambil data telur */
        let telur = await telurModel.findAll() //data telur yang ada didb
        //  ambil data pack
        ...
        let sendData = {
            ...
            page: `form-transaksi`,
            tgl_transaksi: ``,
            dataTelurString: JSON.stringify(telur), //stringify untuk mengubah data array menjadi string
...
exports.simpanTransaksi = async (request, response) => {
    try {
        ...
        /** simpan transaksi */
        let resultTransaksi = await transaksiModel.add(newTransaksi) //menyimpan data transaksi ke database

        /** menampung isi cart */
        let cart = request.session.cart

        for (let i = 0; i < cart.length; i++) { // looping sesuai data yang disimpan dicart
            ...
            cart[i].id_transaksi = resultTransaksi.insertId // menyimpan data id_transaksi ke cart
